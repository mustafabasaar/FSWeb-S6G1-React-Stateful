...
export default function Programcilar() {
  ...
  const [programcılarList, setProgramcılarList] = useState(enIyilerListesi);
  const [eniyiProgramcı, setEniyiProgramcı] = useState(null);
  ...
  const oneCikaninIsmi = () => {
    ...
    if (eniyiProgramcı) {
      const aktifProgramcı = programcılarList.find(
        (dev) => dev.id === eniyiProgramcı
      );
      return aktifProgramcı.isim;
    } else {
      return "";
    }
  };
  ...
  const stil = {
    fontSize: "1.5em",
    marginTop: "0.5em",
    color: eniyiProgramcı ? "gold" : "royalblue", // 🤔 kutlarken renk gold'a dönecek
  };
